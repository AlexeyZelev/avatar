...
class Timeline_chart extends Widget_Base
{
    ...
    private function getDate($new = false)
    {
        if ($new) {
            $currentDate = date('Y-m-d H:i');
        } else {
            $currentDate = $this->preDate;
        }
        $this->preDate = date('Y-m-d H:i', strtotime($currentDate . ' +' . rand(1, 50) . ' days' . ' +' . rand(1, 60) . ' minutes' . ' +' . rand(1, 24) . ' hours'));
        return $this->preDate;
    }
    ...
    protected function _register_controls()
    {
        ...
        graphina_advance_x_axis_setting($this, $type, false);

        graphina_advance_y_axis_setting($this, $type, false);
            ...
            for ($i = 0; $i < $seriesCount; $i++) {
                ...
                foreach ($valueList as $key => $val) {
                    if (count($settings['iq_' . $type . '_category_list']) > $key) {
                        $value[] = [
                            'x' => strval($settings['iq_' . $type . '_category_list'][$key]['iq_' . $type . '_chart_category']),
                            'y' => [
                                strtotime($val['iq_' . $type . '_chart_from_date_' . $i]) * 1000,
                                strtotime($val['iq_' . $type . '_chart_to_date_' . $i]) * 1000
                            ]
                        ];
                    }
                }
                $data['series'][] = [
                    'name' => $settings['iq_' . $type . '_chart_title_3_' . $i],
                    'data' => $value
                ];
            }
